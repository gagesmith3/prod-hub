...
export default function PostProductionTab({ project }) {
  return (
    <Paper shadow="xs" radius="md" p="lg" mb="xl" withBorder>
      <Title order={4} mb="md">Post-Production</Title>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2rem' }}>
        <Section title="Sort Media" />
        <Section title="Download Media Package" />
        <Section title="Upload Cut/File" />
        <Section title="Review Footage" />
        <Section title="Review Cut" />
        <Section title="Remote Render" />
      </div>
    </Paper>
  );
}
